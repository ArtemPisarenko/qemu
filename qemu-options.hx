...
instances. Sockets which cannot work in connected state (such as
AF_INET multicast) require special handling: they must be connect()-ed to
destination address for sendto() before invoking qemu, and, after qemu goes
through preconfig state, they can be unconnected back.
